// GridCellGroup.cpp: implementation of the CGridCellGroup class.
//
//////////////////////////////////////////////////////////////////////

#include "../../../stdafx.h"
#include "GridCellGroup.h"
#include "../GridCtrl.h"
...
IMPLEMENT_DYNCREATE(CGridCellGroup, CGridCell)

CGridCellGroup::CGridCellGroup()
{
...
CSize CGridCellGroup::GetCellExtent( CDC* pDC )
{
	// 上边文字与下边文字取最大的宽度
	CSize tmpSize = GetTextExtent(m_groupInfo.GetEquipmentList(), pDC);
	CSize size = GetTextExtent(m_groupInfo.GroupType, pDC); 
	size.cy *= 2;
...
BOOL CGridCellGroup::Draw( CDC* pDC, int nRow, int nCol, CRect rect, BOOL bEraseBkgnd /*= TRUE*/ )
{
...
//		/rect.bottom++;
		...
		DrawText(pDC->m_hDC, m_groupInfo.GetEquipmentList(), -1, rect, GetFormat() | DT_NOPREFIX | DT_TOP | DT_CENTER | DT_SINGLELINE);		// | DT_CENTER
		CRect rc = rect;
		rc.top = rect.bottom;
		rc.bottom = rc.top + rect.Height();
		//DrawText(pDC->m_hDC, ExtractString(str, 1), -1, rc, GetFormat() | DT_NOPREFIX);
		DrawText(pDC->m_hDC, m_groupInfo.GroupType, -1, rc, GetFormat() | DT_NOPREFIX | DT_TOP | DT_CENTER | DT_SINGLELINE);		// | DT_CENTER
...
void CGridCellGroup::SetGroupInfo( CGroupCellInfo &groupInfo )
{
	m_groupInfo = groupInfo;
}

CGroupCellInfo CGridCellGroup::GetGroupInfo() const
{
	return m_groupInfo;
}

BOOL CGridCellGroup::GetTextRect( LPRECT pRect )
{
